    ...
    <div className="controls-panel">
      ...
      <div className="panel-section">
        ...
        <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
          <button className="sim-btn sim-btn-primary" onClick={() => { store.setSimRunning(true); store.setSimPaused(false); }}>▶ Start</button>
          <button className="sim-btn" onClick={() => store.setSimPaused(!store.simPaused)}>⏸ Pause</button>
          <button className="sim-btn" onClick={() => store.resetSim()}>↺ Reset</button>
          <button className="sim-btn sim-btn-danger" onClick={() => store.setSimRunning(false)}>⏹ Stop</button>
        </div>
      </div>
      ...
      <div className="panel-section">
        ...
        <div style={{ display: 'flex', gap: '3px', marginTop: 4, flexWrap: 'wrap' }}>
          {[0.25, 0.5, 1, 2, 5, 10].map(v => (
            <button key={v} className="sim-btn" style={{ padding: '2px 5px', fontSize: '8px' }} onClick={() => store.setSimSpeed(v)}>{v}x</button>
          ))}
        </div>
      </div>
    </div>
